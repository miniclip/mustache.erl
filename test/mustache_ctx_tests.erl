...
-dialyzer({no_opaque, new_ctx_from_dict_test/0}). % because of the dict as input
...
-dialyzer({nowarn_function, new_ctx_from_improper_data_test/0}). % testing errors on purpose
...
get_from_module_call_order_() ->
    ok = meck:expect(mock_module, key, fun(_) -> value_one end),
    ok = meck:expect(mock_module, key, fun() -> value_zero end),
    Ctx0 = mustache_ctx:new(),
    Ctx1 = mustache_ctx:module(mock_module, Ctx0),
    ?assertEqual({ok, value_one}, mustache_ctx:get(key, Ctx1)).
